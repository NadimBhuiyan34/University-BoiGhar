<?php
    ...
    if(!empty($_POST)){
        ...
        if(!empty($title)){
            $update="UPDATE adm_sechandbook SET secbook_title='$title',secbook_edition='$edition',secbook_price='$price',seller_num='$number' WHERE secbook_id='$id'";
            if(mysqli_query($con,$update)){
                header('Location: view-sechandbook.php?v='.$id);
            }else{
              echo "Update failed!";
            }
        }else{
