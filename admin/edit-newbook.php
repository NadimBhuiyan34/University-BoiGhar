<?php
    ...
    if(!empty($_POST)){
        ...
        if(!empty($title)){
            $update="UPDATE adm_newbook SET book_title='$title',book_edition='$edition',book_price='$price',book_stock='$stock' WHERE id='$id'";
            if(mysqli_query($con,$update)){
                header('Location: view-newbook.php?v='.$id);
            }else{
              echo "Update failed!";
            }
        }else{
